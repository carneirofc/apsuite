"""Keysight Infiniium DSOS104A Oscilloscope."""
...
class KeySight:
    ...
    def capture(self, source):
        ...
        va1 = np.array(list(dataraw)[0::2])
        va0 = np.array(list(dataraw)[1::2])
        datay = ((va1 << 8) + va0 - 2**16*(va1 >> 7)) * yinc + yor

        datax = np.arange(datay.size)*xinc
    ...
    def get_data(self):
        ...
        try:
            ...
            print('Total acquisition time:', time.time()-tini)
